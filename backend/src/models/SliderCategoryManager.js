...
class SliderCategory extends AbstractManager {
  ...
  findAllByCategory() {
    return this.connection
      .query(`select ${this.table}.id, ${this.table}.Number, ${this.table}.id_category, category.Name as category, video.Name as videoName, video.id as id_video, video.Url, video.premium from ${this.table}
  ...
  findByCategory(id, limit) {
    let query = `select ${this.table}.id, ${this.table}.Number, ${this.table}.id_category, category.Name as category, video.id as id_video, video.Name , video.Url, video.Premium, video.Screenshot from ${this.table}
    inner join category on ${this.table}.id_category = category.id
    inner join video on ${this.table}.id_category = video.id_Category
    where ${this.table}.id = ?`;
    const value = [id];
